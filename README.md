...
**Entregable principal:** [`notebooks/flawed_model_squad_draft_v4.ipynb`](notebooks/flawed_model_squad_draft_v4.ipynb).
...
3. **Rehacer el forecast sobre el flujo neto, no sobre el nivel del saldo**, con una escalera de modelos
   de más simple a más complejo (naive estacional → ETS → SARIMAX → Prophet → bootstrap empírico),
   validada con `TimeSeriesSplit` walk-forward sin fuga de información (`src/forecast.py`). Se elige el
   modelo más simple que le gane al baseline: gana el bootstrap empírico, porque ningún modelo paramétrico
   supera su distribución — la señal no está en el punto, está en los cuantiles.
...
`notebooks/flawed_model_squad_draft_v4.ipynb` tiene la Parte 1 completa y la Parte 2 con su núcleo técnico
(forecast, políticas, motor de rebalanceo) — las secciones 2.0–2.2 (formulación, reconciliación del log de
transferencias, estructura de flujos) y la celda de verificación final del Resumen Ejecutivo siguen
pendientes de rehacer en este archivo. Las Partes 3, 4 y 5 están completas.
...
notebooks/        flawed_model_squad_draft_v4.ipynb  <- el entregable (importa de src/, no define lógica)
...
src/forecast.py   F3  escalera de modelos, bootstrap, backtest walk-forward
...
jupyter nbconvert --to notebook --execute --inplace notebooks/flawed_model_squad_draft_v4.ipynb   # ~5-6 min
```
