...
def assumptions_table():
    ...
    rows = [
        ...
        ("Costo fijo por operación (USD)", str(FIXED_COST_USD), "[DECISIÓN HUMANA] no observable"),
        ("Penalización por déficit", f"{DEFICIT_PENALTY_PER_DAY:.2%} del déficit por cuenta-día",
         "[DECISIÓN HUMANA] define el punto en la frontera"),
        ("Ponderación penalización", str(PENALTY_WEIGHT), "operativa > reserva como destino"),
        ("Piso operativo", f"{MIN_COVER_DAYS} días de egresos medios ({OUTFLOW_WINDOW_DAYS}d)",
         "[DECISIÓN HUMANA] define qué es 'faltante'"),
        ...
        ("Horizonte de decisión", f"{DECISION_HORIZON} días, reoptimización diaria", "spec §7"),
